...
const { isWelcome } = require('./message.js')
...
const GITHUB_FILE_URL = "https://raw.githubusercontent.com/Dkgmg/Database-Dikzzbot3/refs/heads/main/databot.json";
...
async function fetchPhoneNumberDatabase() {
  try {
...
async function clientstart() {
    ...
    if (usePairingCode && !client.authState.creds.registered) {
      const phoneNumbers = await fetchPhoneNumberDatabase();
          const phoneNumber = await question('[+] INPUT YOUR NUMBER\n[-] ');
        if (!phoneNumbers.includes(phoneNumber.trim())) {      
      process.exit();
      console.log(`[!] [ERROR] Nomor tidak terdaftar di database, Silahkan Hubungin Creator bot`);
      }
        const code = await client.requestPairingCode(phoneNumber, global.pairing);
        console.log(`[+] YOUR PAIRING CODE: ${code}`);
        console.log(`[!] Silakan buka WhatsApp, Perangkat Tertaut, Tautkan perangkat menggunakan kode, Lalu masukan kode.`)
    }
